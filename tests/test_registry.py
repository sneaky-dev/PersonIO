...
class TestRegistry(unittest.TestCase):
    ...
    def test_filter_formats(self):
        self.assertEqual(set(formats_registry.list(priority=0)), {CSVFormat, JSONFormat})
        self.assertEqual(set(formats_registry.list(extension=".json")), {JSONFormat})
        self.assertEqual(formats_registry.get(extension=".json"), JSONFormat)
        self.assertEqual(formats_registry.get(label="JSON"), JSONFormat)

    def test_filter_printers(self):
        self.assertEqual(set(printers_registry.list(priority=0)), {StdoutPrinter, HTMLPrinter})
        self.assertEqual(set(printers_registry.list(label="HTML")), {HTMLPrinter})